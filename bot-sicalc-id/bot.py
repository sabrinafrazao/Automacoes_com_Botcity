...
def main():
    ...
    btn_continiuar = bot.find_app_element(from_parent_window=janela_esclarecimento, 
                                          title="&Continuar", 
                                          class_name ="ThunderRT6CommandButton" )
    btn_continiuar.click()

    janela_principal = bot.find_app_window(title="Sicalc Auto Atendimento",
                                           class_name="ThunderRT6MDIForm")
    
    janela_principal.menu_select("Funções -> Preenchimento de DARF")

    janela_darf = bot.find_app_element(from_parent_window=janela_principal, 
    ...
    janela_principal = bot.find_app_window(title_re = "Sicalc Auto Atendimento",
                                           class_name="ThunderRT6MDIForm")
    
    form_darf = bot.find_app_element(from_parent_window=janela_principal,
                                     title="Receita",
    ...
    bot.wait(2000)
    form_darf.Edit5.type_keys("1000")
   ...
   # Preenchendo ultimo formulario
    ...
    # Botão de imprimir
    form_darf.type_keys("%{i}")

    save =  bot.find_app_window(title="Salvar Saída de Impressão como")
    save.type_keys(r"C:\Users\sabri\Desktop\TreinamentoBotcity\bot-sicalc\pdfs\Darf2.pdf")
